...
interface FiscalAppointment {
  ...
  scheduled_at?: string
  completed_at?: string
  progress_notes?: AppointmentProgressNote[]
}
...
const formatScheduledLabel = (date?: string, time?: string, period?: string) => {
  if (!date) return 'Sem data agendada'
  ...
  const formattedPeriod = normalizedPeriod ? ` (${normalizedPeriod})` : ''
...
export default function StudentFiscalAppointments({ token }: StudentFiscalAppointmentsProps) {
  ...
  const loadAppointments = useCallback(async (options?: { skipLoadingState?: boolean }) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const appts = (data.fiscalAppointments || []) as FiscalAppointment[]
        const normalizedAppointments = appts.map(appointment => ({
          ...appointment,
          progress_notes: Array.isArray(appointment.progress_notes)
            ? appointment.progress_notes
            : []
        }))
        ...
        setStats({
          ...
          pendentes: normalizedAppointments.filter((a) => a.status === 'PENDENTE').length,
          confirmados: normalizedAppointments.filter((a) => a.status === 'CONFIRMADO').length,
          emAndamento: normalizedAppointments.filter((a) => a.status === 'EM_ANDAMENTO').length,
          concluidos: normalizedAppointments.filter((a) => a.status === 'CONCLUIDO').length,
  ...
  const handleSaveProgressNote = async (appointmentId: string) => {
    const draft = (noteDrafts[appointmentId] || '').trim()

    if (!draft) {
      setNoteErrors(prev => ({
        ...prev,
        [appointmentId]: 'Digite uma anotação antes de salvar.'
      }))
      return
    }

    setNoteSaving(prev => ({ ...prev, [appointmentId]: true }))
    setNoteErrors(prev => ({ ...prev, [appointmentId]: '' }))

    try {
      const response = await fetch('/api/students/fiscal-appointments/notes', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
        },
        body: JSON.stringify({ appointmentId, note: draft })
      })
      ...
      console.error('Erro ao salvar anotação do atendimento:', err)
      setNoteErrors(prev => ({
        ...prev,
        [appointmentId]: err instanceof Error ? err.message : 'Erro desconhecido'
      }))
    } finally {
      setNoteSaving(prev => ({ ...prev, [appointmentId]: false }))
    }
  }
                  ...
                  <div className="space-y-2">
                    ...
                    {appointment.preferred_time && (
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span>
                          Horário: {appointment.preferred_time}
                          {appointment.preferred_period ? ` (${appointment.preferred_period})` : ''}
                        </span>
                      </div>
                    )}
                  </div>
            ...
            <div className="p-6 space-y-6">
                    ...
                    {selectedAppointment.preferred_date && (
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          <p className="text-sm font-medium">Atendimento Agendado</p>
                          <p className="text-xs text-gray-500">
                            {formatScheduledLabel(
                              selectedAppointment.preferred_date,
                              selectedAppointment.preferred_time,
                              selectedAppointment.preferred_period
                            )}
                          </p>
                        </div>
                      </div>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Anotações adicionadas durante o atendimento</CardDescription>
                </CardHeader>
                  ...
                  {selectedAppointment.status === 'EM_ANDAMENTO' && (
                    <div className="border-t border-gray-200 dark:border-gray-800 pt-4 space-y-3">
                      ...
                      <Textarea
                        ...
                        rows={3}
                      />
                      {noteErrors[selectedAppointment.id] && (
                        <p className="text-xs text-red-500">{noteErrors[selectedAppointment.id]}</p>
                      )}
                      <div className="flex justify-end">
                        <Button
                          size="sm"
                          onClick={() => handleSaveProgressNote(selectedAppointment.id)}
                          disabled={noteSaving[selectedAppointment.id]}
                          className="bg-emerald-600 hover:bg-emerald-700"
                        >
                          {noteSaving[selectedAppointment.id] ? (
