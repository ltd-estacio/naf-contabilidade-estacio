  ...
  try {
    ...
    return NextResponse.json({
      ...
      appointment: {
        ...
        preferred_period: appointment.preferred_period,
...
export async function GET(request: NextRequest) {
  try {
    ...
    const appointmentIds = processedAppointments.map(apt => apt.id).filter(Boolean)
    ...
    if (appointmentIds.length > 0) {
      ...
      if (notesError) {
        console.error('Erro ao buscar notas dos atendimentos (coordenador):', notesError)
      } else {
