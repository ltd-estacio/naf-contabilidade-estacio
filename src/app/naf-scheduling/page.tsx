...
const derivePeriodFromTime = (time: string): 'MANHA' | 'TARDE' | 'NOITE' => {
...
const formatScheduledDate = (isoDate?: string | null, time?: string, period?: string | null) => {
  if (!isoDate) return 'Não informado'

  const base = new Date(`${isoDate}T${time || '00:00:00'}`)
  ...
  const normalizedPeriod = period ? periodLabels[period as keyof typeof periodLabels] : undefined

  return `${formattedDate} às ${time}${normalizedPeriod ? ` (${normalizedPeriod})` : ''}`
}
  ...
  const handleDateSelection = (date: Date | undefined) => {
    ...
    setAppointment(prev => {
      ...
      return {
        ...
        preferredDateISO: normalizedDate.toISOString().split('T')[0],
        preferredTime: isSameDay ? prev.preferredTime : '',
        preferredPeriod: isSameDay ? prev.preferredPeriod : null
      }
    })
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const appointmentData = {
        ...
        // Agendamento
        preferredDate: appointment.preferredDateISO,
        preferredTime: appointment.preferredTime,
        preferredPeriod: appointment.preferredPeriod,
      ...
      setAppointment(prev => ({
        ...
        preferredDate: result.appointment?.preferred_date
          ? new Date(`${result.appointment.preferred_date}T${(result.appointment.preferred_time || '00:00:00')}`)
          : prev.preferredDate,
        preferredDateISO: result.appointment?.preferred_date || prev.preferredDateISO,
        preferredTime: result.appointment?.preferred_time || prev.preferredTime,
        preferredPeriod: result.appointment?.preferred_period || prev.preferredPeriod
      }))
      ...
      try {
        ...
        const formattedEmailDate = appointment.preferredDateISO
          ? new Intl.DateTimeFormat('pt-BR', {
              weekday: 'long',
              year: 'numeric',
              month: 'long',
              day: 'numeric'
            }).format(new Date(`${appointment.preferredDateISO}T00:00:00`))
        ...
        const formattedEmailPeriod = appointment.preferredPeriod
          ? periodLabels[appointment.preferredPeriod] || appointment.preferredPeriod
          : 'Turno a definir'
        ...
        const emailResult = await sendAppointmentConfirmationEmail({
          ...
          preferredDate: formattedEmailDate,
          preferredTime: appointment.preferredTime,
          preferredPeriod: formattedEmailPeriod,
  ...
  const renderStep4 = () => (
    <div className="text-center space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Seu atendimento foi agendado com sucesso para
          {' '}
          <strong>{formatScheduledDate(appointment.preferredDateISO, appointment.preferredTime, appointment.preferredPeriod)}</strong>.
        </p>
      </div>

      <div className="inline-flex items-center gap-3 px-4 py-2 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-full text-emerald-700 dark:text-emerald-300">
        <CalendarIcon className="h-4 w-4" />
        <span className="text-sm font-medium">Dia do atendimento confirmado</span>
      </div>
        ...
        <CardContent className="p-6">
          <div className="space-y-4 text-left">
            ...
            <div className="flex justify-between">
              ...
              <span className="dark:text-gray-300">{formatScheduledDate(appointment.preferredDateISO)}</span>
            </div>
            <div className="flex justify-between">
              <span className="font-medium dark:text-gray-300">Horário:</span>
              <span className="dark:text-gray-300">{appointment.preferredTime || 'Não informado'}</span>
            </div>
            <div className="flex justify-between">
