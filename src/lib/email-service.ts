...
class EmailService {
  ...
  private getScheduleConfirmationTemplate(data: unknown): string {
    return `
      <!DOCTYPE html>
      <html>
        <head>
  ...
  private getAttendanceValidationTemplate(data: unknown): string {
    return `
      <!DOCTYPE html>
      <html>
        <head>
  ...
  private getReminderTemplate(data: unknown): string {
    return `
      <!DOCTYPE html>
      <html>
        <head>
  ...
  async sendBatchReminders(appointments: unknown[]): Promise<void> {
